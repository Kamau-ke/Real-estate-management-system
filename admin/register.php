...
if(isset($_POST['submit'])){
   ...
   $select_admins->bind_param("s", $name); // 's' specifies the variable type => 'string'
   $select_admins->execute();

   if ($select_admins->get_result()->num_rows > 0) {
       $warning_msg[] = 'Username already taken!';
   } else {
       if ($pass !== $c_pass) { // Note: you need to fix password hashing as well.
           $warning_msg[] = 'Password not matched!';
       } else {
           $insert_admin = $conn->prepare("INSERT INTO `admins`(name, password) VALUES(?, ?)");
           $insert_admin->bind_param("ss", $name, $pass); // Bind the parameters
