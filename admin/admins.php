...
<section class="grid">
   ...
   <?php
         ...
         $select_admins_res = "SELECT * FROM `admins`" or die($conn->error);
         $select_admins_res=$conn->query( $select_admins_res);
      
      if($select_admins_res){
         while($fetch_admins = $select_admins_res->fetch_assoc()){
   ?>
   <?php if( $fetch_admins['id']){ ?>
