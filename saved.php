...
<section class="listings">
   ...
   <div class="box-container">
      <?php
         $total_images = 0;
         $select_saved_property_query = "SELECT * FROM `saved` WHERE user_id = '$user_id'";
         $select_saved_property_res=$conn->query($select_saved_property_query );

         if($select_saved_property_res->num_rows > 0){
         while($fetch_saved = $select_saved_property_res->fetch_assoc()){

            $fetch_saved_property_id= $fetch_saved['property_id'];


         $select_properties_query = "SELECT * FROM `property` WHERE id = '$fetch_saved_property_id' ORDER BY date DESC";

         $select_properties_res=$conn->query($select_properties_query );
         if($select_properties_res->num_rows > 0){
            while($fetch_property = $select_properties_res->fetch_assoc()){
               $fetch_property_user_id=$fetch_property['user_id'];

            $select_user_query = "SELECT * FROM `users` WHERE id = '$fetch_property_user_id'";
